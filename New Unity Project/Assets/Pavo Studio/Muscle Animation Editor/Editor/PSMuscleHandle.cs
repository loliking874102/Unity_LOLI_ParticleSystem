...
namespace PavoStudio.MAE
{
    ...
    class PSMuscleHandle
    {
            ...
            if (selected > -1)
            {
                ...
                if (selected >= 1000)
                {
                    ...
                    if (gid < 0 || gid >= PSMuscleDefine.TQGroup.Length)
                        return;
                    GUILayout.Window(1, new Rect(Screen.width - 220, Screen.height - 180, 200, 100), RootGUI, PSMuscleDefine.TQGroup[gid]);
                }
                else
        ...
        private void DrawSlider(int i, Color color)
        {
            ...
            float val = GUILayout.HorizontalSlider(tabMuscle.muscleValue[i], -1, 1, GUILayout.Height(20));
